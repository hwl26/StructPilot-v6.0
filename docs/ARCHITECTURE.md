...
│  StructPilotApp：navigator → memory 单线图                  │
│  挂载点 _polish_reply：RAG 检索 + vision 图片 + LM 改写       │
├──────────────────────────────────────────────────────┤
│  智能体层  agents/                                          │
│  NavigatorAgent（状态机，唯一权威应答者）                    │
│  ExpertAgent / SOPAgent（当前为冗余备份，未接入路由）         │
...
### 1. 单一权威应答者
`NavigatorAgent` 是唯一追加助手回复的节点。`_route_from_navigator` 永远走 `memory`，
expert / sop / fault 节点保留但不再触发。这根治了早期「一次输入产生两条回复」的 bug，
...
| 编排层 | `graph/app.py` | `StructPilotApp`，LangGraph 单线图，`_polish_reply` 挂载 RAG/vision/改写 |
...
| 智能体 | `agents/expert_agent.py` `agents/sop_agent.py` | 当前为冗余备份，未接入路由 |
...
| 多 Agent 决策 | 降级为单 navigator | expert/sop 未接入 | 架构（有意为之） |
| 记忆库 - 模式 1（代码预写） | 完整 | 可用 | — |
...
- **P1 工程健壮性**：`tests/` 为空，建议补状态机流转、RAG 降级、序列化往返的单元测试；
  ExpertAgent / SOPAgent 为「死节点」，建议删除或等知识库充实后真正接入。
