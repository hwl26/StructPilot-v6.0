...
_LAB_EXP_PATH = os.path.join(_KNOWLEDGE_DIR, "lab_experience_kb.json")
...
def _lab_exp_to_text(exp: Dict[str, Any]) -> str:
    """Flatten an approved lab experience into retrieval text."""
    parts: List[str] = []
    for key in ("title", "symptoms_text", "solution", "lesson"):
        value = exp.get(key)
        if isinstance(value, str) and value.strip():
            parts.append(value.strip())

    for key in ("symptoms", "tags"):
        value = exp.get(key)
    ...
    related_params = exp.get("related_params")
    if isinstance(related_params, dict) and related_params:
        parts.append(json.dumps(related_params, ensure_ascii=False, sort_keys=True))
    elif isinstance(related_params, list):
        parts.append("; ".join(str(item) for item in related_params if str(item).strip()))
...
def _load_lab_experience() -> List[Tuple[str, str, float]]:
    """Load approved lab experiences with the lab-tier priority boost."""
    if not os.path.exists(_LAB_EXP_PATH):
        return []
    try:
        with open(_LAB_EXP_PATH, "r", encoding="utf-8") as f:
            data = json.load(f)
        entries = data.get("entries", []) if isinstance(data, dict) else []
        if not isinstance(entries, list):
            return []
        weight = min(TIER_WEIGHTS.get("lab", 0.92) * 1.10, 0.98)
        corpus: List[Tuple[str, str, float]] = []
        for exp in entries:
            if not isinstance(exp, dict):
                continue
            doc_id = str(exp.get("id") or "").strip()
            text = _lab_exp_to_text(exp)
            if doc_id and text.strip():
                corpus.append((doc_id, text, weight))
        return corpus
    except Exception:
        return []
...
class KnowledgeRetriever:
    ...
    def build_corpus(self, force_rebuild: bool = False) -> List[Tuple[str, str, float]]:
        ...
        with self._corpus_lock:
            ...
            corpus.extend(_load_lab_experience())
    ...
    def search(self, query: str, top_k: int = 3,
        ...
        try:
            ...
            candidate_count = min(len(weighted_scores), max(top_k * 3, top_k))
            order = list(np.argsort(-weighted_scores, kind="stable")[:candidate_count])

            # Keep score order, but prefer lab experience inside near-tie groups.
            cursor = 0
            while cursor < len(order):
                group_end = cursor + 1
                anchor_score = float(weighted_scores[order[cursor]])
                while (
                    group_end < len(order)
                    and anchor_score - float(weighted_scores[order[group_end]]) < 0.02
                ):
                    group_end += 1
                order[cursor:group_end] = sorted(
                    order[cursor:group_end],
                    key=lambda idx: 0 if corpus[idx][0].startswith("lab_") else 1,
                )
                cursor = group_end
            order = order[:top_k]
            results = [
