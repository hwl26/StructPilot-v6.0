...
class LLMAgent:
    ...
    def _system_prompt(self, response_profile: str = "teaching") -> str:
        return (
            ...
            "【课题组经验优先原则】\n"
            "当检索结果包含 source_type=lab_exp 的课题组经验时：\n"
            "1. 优先引用，回答中标注“课题组{author}（{author_role}）于{date}复盘：...”。\n"
            "2. 引用经验时包含“问题→解决思路→心得”三段式。\n"
            "3. 若经验与官方文档冲突，先呈现经验，再补充官方建议。\n"
            "4. 经验的 lesson 字段用“💡 课题组沉淀：{lesson}”格式在回答结尾高亮展示。\n"
            "5. 若多条经验都相关，按 step 字段排序后依次呈现。\n"
